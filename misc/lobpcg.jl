...
function solve_lobpcg(H,n,l,k_list_squared;maxiter=100,tol=1e-5,X0=-1,full_diag=false,print_infos=false) # Finds the first l eigenmodes of the action H. Full diagonalization if the dimension is too low
	if X0 == -1
	...
	if n > 30 && !full_diag
		...
		if print_infos
			...
			res = lobpcg_hyper(H,X0,k_list_squared; maxiter=maxiter, tol=tol)
		end
		if !res.converged
			px("LOBPCG DID NOT CONVERGE IN ",res.iterations," STEPS")
		end
